      ...
      <el-table :data="projects" v-loading="loading" border stripe>
        ...
        <el-table-column label="来源" min-width="90">
          <template #default="{ row }">
            ...
            <el-tag v-else type="info" size="small">App自动化</el-tag>
          </template>
        </el-table-column>
        <el-table-column :label="$t('appAutomation.common.operation')" min-width="150">
          <template #default="{ row }">
            ...
            <el-button type="warning" link size="small" :disabled="row.source === 'ai'" @click="openEditDialog(row)">{{ $t('appAutomation.common.edit') }}</el-button>
            <el-button type="danger" link size="small" :disabled="row.source === 'ai'" @click="handleDelete(row)">{{ $t('appAutomation.common.delete') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
