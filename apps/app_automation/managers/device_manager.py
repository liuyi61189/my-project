...
class DeviceManager:
    ...
    def __init__(self, adb_path='adb', adb_host=None, adb_port=None):
        self.adb_path = adb_path
        # adb_host: 远程ADB服务地址（如 192.168.8.71），None 则用默认
        # 支持 "IP:PORT" 形式（如 192.168.8.71:5038），会自动拆分出端口
        self.adb_port = None
        if adb_host and ':' in str(adb_host):
            host_part, _, port_part = str(adb_host).partition(':')
            self.adb_host = host_part
            try:
                self.adb_port = int(port_part)
            except ValueError:
                self.adb_port = None
        else:
            self.adb_host = adb_host
        # 显式传入的 adb_port 优先级最高
        if adb_port:
            try:
    ...
    def _build_adb_args(self, extra_args=None):
        """构建 ADB 命令参数列表（自动加 -H 和 -P）"""
        args = [self.adb_path]
        if self.adb_host:
            args.extend(['-H', str(self.adb_host), '-P', str(self.adb_port)])
        if extra_args:
    ...
    def list_devices(self):
        ...
        try:
            ...
            cmd_args = self._build_adb_args(['devices', '-l'])
            logger.info(f"执行ADB命令: {' '.join(cmd_args)}")
            result = subprocess.run(
                cmd_args,
                capture_output=True,
                text=True,
    ...
    def get_device_info(self, device_id):
        ...
        try:
            # 获取设备名称
            result = subprocess.run(
                self._build_adb_args(['-s', device_id, 'shell', 'getprop', 'ro.product.model']),
                capture_output=True,
                text=True,
                timeout=5,
                **self.subprocess_kwargs
            ...
            # 获取Android版本
            result = subprocess.run(
                self._build_adb_args(['-s', device_id, 'shell', 'getprop', 'ro.build.version.release']),
                capture_output=True,
                text=True,
                timeout=5,
                **self.subprocess_kwargs
    ...
    def connect_device(self, ip_address, port=5555):
        ...
        try:
            # 执行连接命令
            result = subprocess.run(
                self._build_adb_args(['connect', device_address]),
                capture_output=True,
                text=True,
                timeout=30,
                **self.subprocess_kwargs
            ...
            logger.info(f"连接结果 stdout={output}, stderr={stderr}, rc={result.returncode}")

            if result.returncode != 0:
                error_msg = stderr or output or '未知错误'
                logger.error(f"连接失败(rc={result.returncode}): {error_msg}")
                # 区分网络类错误（客户端问题）和内部错误
                if any(kw in error_msg.lower() for kw in ['connection refused', 'connection timed out',
                                                           'no route to host', 'network is unreachable',
                                                           'host unreachable']):
                    raise ConnectionError(f"无法连接到 {device_address}: {error_msg}")
            ...
            if 'connected' in output.lower() or 'already connected' in output.lower():
                ...
                # ADB 返回 "failed to connect to..." 也视为网络错误
                if any(kw in output.lower() for kw in ['connection refused', 'failed to connect',
                                                        'cannot connect to']):
                    raise ConnectionError(f"无法连接到 {device_address}: {output}")
                raise Exception(f"连接失败: {output}")

        except subprocess.TimeoutExpired:
            logger.error("连接超时")
            raise ConnectionError(f"连接 {device_address} 超时，请检查设备网络是否通畅")
        except ConnectionError:
            raise  # 网络类错误直接向上抛出，由视图层决定HTTP状态码
        except Exception as e:
    ...
    def disconnect_device(self, device_id: str) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                self._build_adb_args(['disconnect', device_id]),
                capture_output=True,
                text=True,
                timeout=10,
                **self.subprocess_kwargs
