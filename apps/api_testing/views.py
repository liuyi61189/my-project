...
class ApiCollectionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return ApiCollection.objects.filter(
            project__in=filter_by_owner_or_member(ApiProject.objects.all(), user)
        ).distinct()
...
class ApiRequestViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return ApiRequest.objects.filter(
            collection__project__in=filter_by_owner_or_member(ApiProject.objects.all(), user)
        ).distinct()
...
class RequestHistoryViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return RequestHistory.objects.filter(
            request__collection__project__in=filter_by_owner_or_member(ApiProject.objects.all(), user)
        ).select_related(
...
class TestSuiteRequestViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return TestSuiteRequest.objects.filter(
            test_suite__project__in=filter_by_owner_or_member(ApiProject.objects.all(), user)
        ).distinct()
...
class NotificationLogViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        accessible_api_projects = filter_by_owner_or_member(ApiProject.objects.all(), user)
        return NotificationLog.objects.filter(
            models.Q(
                task__test_suite__project__in=accessible_api_projects
            ) | models.Q(
                task__api_request__collection__project__in=accessible_api_projects
            ) | models.Q(
                task__created_by=user
            )
        ).distinct()
...
class ApiDashboardViewSet(viewsets.ViewSet):
    ...
    def stats(self, request):
        ...
        # 获取用户可访问的项目ID列表
        accessible_projects = filter_by_owner_or_member(ApiProject.objects.all(), user)
