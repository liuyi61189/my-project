...
class TestCase(models.Model):
    ...
    feature_modules = models.ManyToManyField(FeatureModule, blank=True, related_name='testcases', verbose_name='关联功能模块')
    test_point = models.ForeignKey(TestPoint, on_delete=models.SET_NULL, null=True, blank=True, related_name='testcases', verbose_name='关联测试点')
    ...
    def latest_execution_result(self):
        """获取最新执行结果，从未执行返回 None"""
        latest = self.executions.first()
        return latest.result if latest else None
...
class TestCaseExecution(models.Model):
    ...
    testcase = models.ForeignKey(TestCase, on_delete=models.CASCADE, related_name='executions')
    executed_by = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name='执行人')
    result = models.CharField(max_length=10, choices=RESULT_CHOICES, verbose_name='执行结果')
